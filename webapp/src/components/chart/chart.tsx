...
const transformCardsToChartData = (
    ...
    const numberProperty = board.cardProperties.find((p) => p.type === 'number')
...
const Chart = (props: Props): JSX.Element => {
    const {board, cards, showCard} = props

    // Unused props kept for interface consistency
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const {activeView, readonly, onCardClicked, addCard} = props

    // 转换卡片数据为图表数据
    const chartData = useMemo(
        () => transformCardsToChartData(cards, board),
        [cards, board],
    )
